...
where C: Cursor, T: Bits {
	...
	/// Gets a write handle to the bit at the given index.
	///
	/// # Parameters
	///
	/// - `&mut self`
	/// - `index: usize`: The bit index to check out. It must be in the domain
	///   `0 .. self.len()`.
	///
	/// # Returns
	///
	/// A guard over the slice, which will commit its value back to the
	/// requested bit when it goes out of scope. This guard handle can be
	/// treated as a `bool` reference for its duration.
	///
	/// # Pancis
	///
	/// This method panics if `index` is outside the slice domain.
	...
	/// use bitvec::*;
	///
	/// let store: &mut [u8] = &mut [4u8];
	/// let bv: &mut BitSlice = store.into();
	/// assert!(*bv.at(5));
	/// *bv.at(5) = false;
	/// assert!(!*bv.at(5));
	/// ```
	pub fn at(&mut self, index: usize) -> BitGuard<T> {
		let len = self.len();
		assert!(index < len, "Index out of range: {} >= {}", index, len);

		let (elt, bit) = self.bitptr().head().offset::<T>(index as isize);

		BitGuard::new(&mut self.as_mut()[elt as usize], C::at::<T>(bit))
	}
...
impl<'a, T> BitGuard<'a, T>
where T: 'a + Bits {
	/// Constructs a new guard from an element reference and bit position.
	pub fn new(elt: &'a mut T, bit: BitPos) -> Self {
		let val = elt.get_at(bit);
		Self {
			elt,
			bit,
			val,
...
/// Enable reading from the guard as an `&bool`.
impl<'a, T> Deref for BitGuard<'a, T>
where T: 'a + Bits {
	type Target = bool;
...
/// Enable writing to the guard as an `&mut bool`.
impl<'a, T> DerefMut for BitGuard<'a, T>
where T: 'a + Bits {
...
/// Enable write-back of the handle’s value to the slice.
impl<'a, T> Drop for BitGuard<'a, T>
where T: 'a + Bits {
	fn drop(&mut self) {
		self.elt.set_at(self.bit, self.val);
	}
}
