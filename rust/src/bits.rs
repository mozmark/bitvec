...
managing bit storage of a fundamental, and is the constraint for the storage
type of the data structures of the rest of the crate.
!*/
...
This trait must only be implemented on unsigned integer fundamentals with full
	...
	+ UpperHex
{
	...
	//  TODO(myrrlyn): Break into an external trait, if not external crate
	...
	///
	/// - `&mut self`
	/// - `place`: A bit index in the element, which will be translated into a
	///   bit position by the `C: Cursor` type parameter. The bit under this
	///   index will be set according to `value`.
	...
	///
	/// - `&mut self`
	/// - `place`: A precomputed bit position in the element. The bit under this
	///   position will be set according to `value`.
	/// - `value`: A Boolean value, which sets the bit on `true` and unsets it
	///   on `false`.
	///
	/// # Panics
	...
	/// # Parameters
	///
	/// - `place`: A bit index in the element, which will be translated by the
	///   `C: Cursor` type parameter. The bit under this index will be retrieved
	///   as a `bool`.
	///
	/// # Returns
	///
	...
	/// # Examples
	///
	/// This example gets three bits from a byte.
	///
	/// ```rust
	/// use bitvec::{Bits, LittleEndian};
	...
	/// Gets a specific bit in an element, at a precomputed position.
	///
	/// # Parameters
	///
	/// - `place`: A precomputed bit position in the element. The bit under this
	///   position will be retrieved as a `bool`.
	///
	/// # Returns
	///
	/// The value of the bit under `place`, as a `bool`.
...
impl BitIdx {
	/// Checks if the index is valid within a type.
	///
	/// The index is valid when it is in the range `0 .. T::SIZE`.
	///
	/// # Parameters
	///
	/// - `self`: The index to validate as a head..
	///
	/// # Returns
	///
	/// Whether the index is valid as a head for the storage type in question.
	///
	/// # Type Parameters
	///
	...
	/// Checks if the index is a valid tail index within a type.
	///
	/// The index is valid when it is in the range `1 ..= T::SIZE`.
	///
	/// # Parameters
	///
	...
	/// Increments an index to the next value, wrapping if needed.
	///
	/// # Parameters
	///
	/// - `self`: The original index.
	///
	/// # Returns
	///
	/// - `Self`: An incremented index.
	...
	/// Decrements a index to the previous value, wrapping if needed.
	///
	/// # Parameters
	///
	/// - `self`: The original index.
	...
	pub fn span<T: Bits>(self, len: usize) -> (usize, BitIdx) {
		//  Number of bits in the head *element*. Domain 32 ..= 1.
		...
		//  the tail will be `T::SIZE`, which is valid for a tail.
		if len <= bits_in_head {
		...
		//  `len` and use the difference to count elements and bits.
			...
			//  become `T::SIZE`; if it is nonzero, add one more to `elts`.
...
/** Newtype indicating a concrete bit position inside an element.
...
mod tests {
	...
	#[test]
	fn jump_far_up() {
		//  `isize::max_value()` is `0x7f...ff`, so the result bit will be one
		//  less than the start bit.
		for n in 1 .. 8 {
	...
	#[test]
	fn jump_far_down() {
		//  `isize::min_value()` is `0x80...00`, so the result bit will be equal
		//  to the start bit
		for n in 0 .. 8 {
