...
template<typename T>
struct BitPtr<T> {
  size_t ptr_head : __builtin_ctzll(sizeof(T));
...
Aligned pointers to `T` do not need their lowest bits; for each power of 2 size
above a byte, another low bit is unused. An aligned pointer to `u16` does not
use its LSb; an aligned pointer to `u64` does not use its lowest three bits. The
unused bits can be used for other fields, and masked away in order to restore
the correct pointer value. It is stored in the high bits of the `ptr` field,
running from MSb down to (inclusive)
`core::mem::size_of::<T>().trailing_zeros()`.

## Element Counter

The memory representation stores a counter that runs from
`0 ..= Self::MAX_ELTS` to indicate how many elements `T` are covered by the
slice. This value may be zero, to indicate an empty slice. It is stored in the
high bits of the `len` field, running from `MSb` down to (inclusive)
`core::mem::size_of::<T>().trailing_zeros() + 6`.

## Head Bit Counter

For any fundamental type `T`, `core::mem::size_of::<T>().trailing_zeros() + 3`
bits are required to count the bit positions inside it. It takes three bits to
select a bit inside a byte, and the remaining bits select a byte inside an
element `T`.

|Type |Size|Trailing Zeros|Count Bits|
|:----|---:|-------------:|---------:|
|`u8` |   1|             0|         3|
|`u16`|   2|             1|         4|
|`u32`|   4|             2|         5|
...
`core::mem::size_of::<T>().trailing_zeros() + 6` down to (inclusive) `3`. The
...
The empty slice is canonically represented by a dangling pointer with zeroed
element, head, and tail counters:

- `data`: `NonNull::<T>::dangling()`
- `elts`: `0usize`
- `head`: `0u8`
- `tail`: `0u8`
- `ptr`: `NonNul::<u8>::dangling()`
- `len`: `0usize`

A wholly zeroed slot – null pointer, empty lengths – is not a valid value of the
`BitPtr<T>` type. This allows `Option<BitPtr<T>>` to take advantage of the
null-value optimization, and encode the zeroed slot value as `None`.
...
the slot are zeroed. Canonically, the empty slice always uses the lowest valid
address for its pointer value, while the uninhabited slices may use any valid
address. The distinction between empty and uninhabited is maintained in the type
system.
...
element unavailable for storage.

# Type Parameters

- `T: Bits` is the fundamental storage type over which the pointer governs.
...
The `BitPtr` type is concerned solely with addressing memory, and has no
awareness of the validity of an address in any given allocation regime. This
module makes no checks or assumptions about whether an address is valid to
dereference.
...
where T: Bits {
	...
	/// not eight bits wide.
	pub const LEN_HEAD_BITS: usize = 8usize.trailing_zeros() as usize;
	/// Marks the bits of `self.len` that are the `head` section.
	pub const LEN_HEAD_MASK: usize = (1 << Self::LEN_HEAD_BITS) - 1;
	...
	///
	/// This is an inclusive ceiling, not an exclusive.
	pub const MAX_ELTS: usize = (1 << Self::LEN_DATA_BITS) - 1;
	...
	///   - in `1 .. T::SIZE` when `elts` is `Self::MAX_ELTS`.
	///
	/// # Returns
	///
	...
	pub fn new<Head: Into<BitIdx>, Tail: Into<BitIdx>>(
		...
		//  slice domain wraps.
		//
		//  Note: the Rust documentation indicates that pointer arithmetic that
		//  may cross allocation boundaries should be done using `usize` rather
		//  than `*T`, in order to prevent the compiler from detecting UB.
		if (data as usize).checked_add(elts).is_none() {
	...
	pub unsafe fn incr_tail(&mut self) {
		let (data, elts, head, tail) = self.raw_parts();
		//  tail is always in `1 ..= T::SIZE`, but `BitIdx` operates in
		//  `0 .. T::SIZE`. Decrement, operate, and re-increment, to achieve the
		//  desired results.
...
mod tests {
	...
	#[test]
	fn full() {
		let elt_ct = BitPtr::<u64>::MAX_ELTS;
	...
	#[should_panic]
	fn overfull() {
		BitPtr::<u64>::new(8 as *const u64, BitPtr::<u64>::MAX_ELTS, 0, 64);
	}
}
