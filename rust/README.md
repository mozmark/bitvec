...
`BitVec` is generic over an ordering cursor, using the trait `Cursor`, and a
fundamental type, using the trait `Bits`. This means that `BitVec` structures
can be built with a great deal of flexibility over how they manage their memory
and translate between the in-memory representation and their semantic contents.
...
> Many methods on the integer fundamentals stabilized as `const fn` in `1.33`.
> This enabled them to be used in `const` bindngs, rather than requiring manual
> definition.
...
- `BitBox<C: Cursor, T: Bits>` - a `BitSlice<C, T>` that owns its memory. This
  type does not have very much unique functionality, as it is essentially a
  `BitSlice` that deallocates.
...
- Make `bitvec!` a procedural macro so that it can condense source code into
  correct static data at compile time.
- Serde support
